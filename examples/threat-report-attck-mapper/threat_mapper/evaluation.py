...
def evaluate_predictions(predictions: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "retrieval": {
            "hit_at_1": _mean(
                float(bool(ids(row, "retrieval")) and ids(row, "retrieval")[0] in set(row["gold_ids"]))
                for row in eligible
            ),
            "hit_at_5": _mean(
                float(bool(set(ids(row, "retrieval")[:5]).intersection(row["gold_ids"]))) for row in eligible
            ),
        ...
        "rerank": {
            "cases": len(reranked),
            "hit_at_1": _mean(
                float(ids(row, "rerank")[0] in set(row["gold_ids"])) for row in reranked if ids(row, "rerank")
            ),
            "hit_at_5": _mean(
                float(bool(set(ids(row, "rerank")[:5]).intersection(row["gold_ids"]))) for row in reranked
            ),
        },
        "verification": {
            "cases": len(verified),
