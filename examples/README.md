...
| [Write public status updates from internal incident reports](./chat) | Checking a generated status update for leaked hosts, tickets, staff names and the wrong outage window | `chat/completions` | SIE Cloud with Qwen3.8 27B; recorded run scores offline with no key | Runnable recorded example |
| [Build a citable graph from filing text](./knowledge-graph) | Turning SEC, NHTSA and FDA paragraphs into entities and relations with offsets, and seeing where the relations go wrong | `extract` | SIE Cloud with GLiNER2; recorded run scores offline with no key | Runnable recorded example |
