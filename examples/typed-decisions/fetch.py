#!/usr/bin/env python3
"""Download the recorded typed-decisions evidence from a pinned dataset revision.
...
def main() -> int:
    ...
    if not REVISION or not MANIFEST_SHA256:
        print(
            f"The {TASK} evidence is not published to {DATASET} yet, so there is nothing to fetch.\n"
            "Build the inputs with build_inputs.py and record with run.py --record; see the README.",
            file=sys.stderr,
        )
        return 1
