...
def convert_images_for_json(images: Sequence[Any]) -> list[dict[str, str]]:
    ...
    """
    if not 1 <= len(images) <= _MAX_GENERATE_IMAGES:
        msg = f"images must contain between 1 and {_MAX_GENERATE_IMAGES} entries"
        raise ValueError(msg)
    converted = convert_item_images({"images": list(images)}).get("images", [])
    if any(not 1 <= len(image["data"]) <= _MAX_GENERATE_IMAGE_BYTES for image in converted):
        msg = "each image must contain between 1 byte and 16 MiB"
        raise ValueError(msg)
    return [
        {
