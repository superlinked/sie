...
class TestGenerateEndpoint:
    ...
    def test_sdk_multimodal_grammar_snippet_runs_against_local_route(
        ...
        config = _make_config(grammar=["json_schema"])
        ...
        async def render(_config: object, prompt: str, image_count: int) -> str:
            assert prompt == "Extract the title"
            assert image_count == 1
            return "<image>Extract the title"

        monkeypatch.setattr("sie_server.api.generate._render_native_image_prompt", render)
        sdk = SIEClient("http://testserver")
        sdk._client.close()
        sdk._client = client  # type: ignore[assignment]
        ...
        assert result["model"] == "Qwen/Qwen3-4B-Instruct"
        assert fake_adapter.last_call is not None
        assert fake_adapter.last_call["prompt"] == "<image>Extract the title"
        assert fake_adapter.last_call["images"] == [{"data": b"\x89PNG\r\n\x1a\npayload", "format": "png"}]
        assert fake_adapter.last_call["grammar"] == GrammarSpec(
            kind="json_schema",
            value={
                "type": "object",
                "properties": {"title": {"type": "string"}},
