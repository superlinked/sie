...
extern "C" int run_ln(
    ...
    // Candle's production device uses CUDA's legacy default stream (a null
    // driver stream). This translation unit is compiled with PTDS semantics,
    // so make the legacy intent explicit instead of interpreting null as PTDS.
    launch_params.stream = stream == nullptr
        ? cudaStreamLegacy
    ...
    auto status = (*launcher)(launch_params, true);
    if (status != cudaSuccess) {
        return status;
    }

    // Launch the kernel.
    status = (*launcher)(launch_params, false);
    if (status != cudaSuccess) {
        return status;
    }
