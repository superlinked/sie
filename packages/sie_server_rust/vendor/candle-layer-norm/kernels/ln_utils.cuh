...
inline cudaError_t check_cuda_(cudaError_t status, const char *file, int line) {
...
#define CHECK_CUDA(ans)                                                                                                        \
    {                                                                                                                          \
        cudaError_t status_ = check_cuda_((ans), __FILE__, __LINE__);                                                          \
...
#define REGISTER_FWD_LAUNCHER(HIDDEN_SIZE, WTYPE, ITYPE, RTYPE, OTYPE, CTYPE, CTAS_PER_ROW, WARPS_M, WARPS_N, BYTES_PER_LDG)                 \
    cudaError_t ln_fwd_##HIDDEN_SIZE##_##WTYPE##_##ITYPE##_##RTYPE##_##OTYPE##_##CTYPE(LaunchParams<FwdParams> &launch_params,                \
                                                                                       const bool configure_params) {                         \
        return launch_<WTYPE, ITYPE, RTYPE, OTYPE, CTYPE, uint32_t, HIDDEN_SIZE, CTAS_PER_ROW, WARPS_M, WARPS_N, BYTES_PER_LDG>(              \
