...
fn main() -> Result<()> {
    ...
    println!("cargo:rerun-if-env-changed=CANDLE_LAYER_NORM_BUILD_DIR");
    ...
    let stamp_file = build_dir.join("liblayernorm.stamp");
    ...
    if should_compile {
        ...
        std::fs::write(&stamp_file, build_stamp)
            .with_context(|| format!("write layer-norm build stamp {}", stamp_file.display()))?;
    }
