...
fn ensure_dense_layout(layout: &Layout, name: &str) -> Result<()> {
    if layout.contiguous_offsets().is_none() {
        candle::bail!("`{name}` has to be contiguous for cuBLASLt")
    }
    Ok(())
}
...
impl CublasLt {
    pub fn new(device: &Device) -> Result<Self> {
        ...
        let inner =
            CudaBlasLT::new(dev.cuda_stream()).map_err(|e| candle::Error::Cuda(Box::new(e)))?;
...
impl CublasLTMatmul {
    pub fn fwd_f16(
        ...
        ensure_dense_layout(a_l, "a")?;
        ensure_dense_layout(b_l, "b")?;
        ...
        let bias = if let (Some(bias), Some(bias_l)) = (bias, bias_l) {
            ...
            ensure_dense_layout(bias_l, "bias")?;
...
/// * `out` - Optional Output tensor of size NxM.
...
/// * `out` - Optional Output tensor of size BxNxM.
///           If set and beta != 0, will be added to the end result of A*B before `act`
/// * `alpha` - Optional scaling factor for A*B
/// * `beta` - Optional scaling factor for C
/// * `bias` - Optional bias tensor of size M
