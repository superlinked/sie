...
>(
    ...
    let (q_ptr, q_sync) = q.view_ptr(&stream);
    let (k_ptr, k_sync) = k.view_ptr(&stream);
    let (cc_ptr, cc_sync) = cc.view_ptr(&stream);
    let (sc_ptr, sc_sync) = sc.view_ptr(&stream);
    ...
    drop((q_sync, k_sync, cc_sync, sc_sync));
    Ok(())
}
