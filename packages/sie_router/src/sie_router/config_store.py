...
class EpochCASError(Exception):
    """Epoch CAS failed — another writer bumped the epoch concurrently."""

    def __init__(self, expected: int, actual: int) -> None:
        self.expected = expected
        self.actual = actual
        super().__init__(f"Epoch CAS failed: expected {expected}, got {actual}")


class ConfigStore:
    """Config store with epoch CAS, backed by local filesystem, S3, or GCS.

    The backend is auto-detected from the base_dir URL scheme:
    - Local path (e.g., /tmp/config-store) → LocalBackend with fcntl CAS
    - s3://bucket/prefix → S3Backend with ETag-based conditional writes
    - gs://bucket/prefix → GCSBackend with generation-based conditional writes
    ...
    def cas_epoch(self, expected: int) -> int:
        """Compare-and-swap the epoch value.

        Atomically increments epoch from expected to expected+1.
        - Local backend: uses fcntl advisory file locking.
        - S3: uses ETag-based conditional PutObject.
        - GCS: uses generation-based conditional upload.

        Args:
            expected: Expected current epoch value.

        Returns:
            New epoch value (expected + 1) on success.

        Raises:
            EpochCASError: If current epoch != expected (concurrent writer).
        """
        expected_content = str(expected) if expected > 0 else ""
        new_epoch = expected + 1

        success = self._backend.write_text_if_match(
            self._epoch_path,
            str(new_epoch),
            expected_content,
        )
        if not success:
            # Read actual epoch for the error message
            actual = self.read_epoch()
            raise EpochCASError(expected, actual)
